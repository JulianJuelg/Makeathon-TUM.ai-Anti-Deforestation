...
def cross_validate_classifier(
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(cv.split(x, y, groups), start=1):
        ...
        y_prob = predict_proba_positive(model, x[val_idx])
...
def run_train(args: argparse.Namespace) -> None:
    ...
    if args.save_train_preview:
        log("save_train_preview is not implemented in this linear script; skipping.")
...
def run_predict(args: argparse.Namespace) -> None:
    ...
    if linear_path.exists():
        ...
        raise FileNotFoundError(f"No saved model found in {output_dir}; expected {linear_path.name}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="AEF temporal-pairs pipeline with linear classifier")
    ...
    def add_trainish(p: argparse.ArgumentParser):
        p.add_argument("--classifier", default="linear", choices=["linear"])
