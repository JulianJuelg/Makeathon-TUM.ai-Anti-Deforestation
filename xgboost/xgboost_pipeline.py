...
) -> tuple[np.ndarray | None, list[str], np.ndarray, np.ndarray, np.ndarray]:
...
def build_training_table(
    ...
    for tile_i, tile_id in enumerate(tile_ids):
        for comparison_year in comparison_years_for_tile(year_map.get(tile_id, []), baseline_year):
            ...
            target, sources_used, pre2020_mask, disagreement_mask, uncertainty_mask = read_training_target(
            ...
            idx = stratified_sample_indices(
                y_all,
                max_pos=max_pos_per_pair,
                max_neg=max_neg_per_pair,
                seed=seed + 1000 * tile_i + comparison_year,
            )
            if len(idx) == 0:
                continue
            ...
            log(
                ...
                f"disagreement_ignored={disagreement_count:,}, uncertainty_ignored={uncertainty_count:,}, "
...
def make_xgb(seed: int, n_estimators: int = 500, scale_pos_weight: float = 1.0):
    if xgb is None:
        raise ImportError("xgboost is not installed; use --classifier linear or install xgboost")
...
def make_classifier(classifier: str, seed: int, y_train: np.ndarray | None = None, n_estimators: int = 500):
    if classifier == "xgboost":
        spw = _scale_pos_weight(y_train) if y_train is not None else 1.0
        return make_xgb(seed, n_estimators=n_estimators, scale_pos_weight=spw)
...
def cross_validate_classifier(
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(cv.split(x, y, groups), start=1):
        model = make_classifier(classifier, seed + fold_idx, y_train=y[train_idx], n_estimators=500)
...
def summarize_cv_metrics(fold_df: pd.DataFrame) -> pd.DataFrame:
    return pd.DataFrame([{
        ...
        "iou_mean": float(fold_df["iou"].mean()),
        "iou_std": float(fold_df["iou"].std(ddof=0)),
    }])


def train_final_model(x: np.ndarray, y: np.ndarray, seed: int, classifier: str, n_estimators: int | None):
    model = make_classifier(classifier, seed, y_train=y, n_estimators=int(n_estimators or 500))
    model.fit(x, y)
    return model
...
def feature_importance_df(model) -> pd.DataFrame:
    if xgb is not None and isinstance(model, xgb.XGBClassifier):
        ...
        return pd.DataFrame({
            "feature": [f"feature_{i:03d}" for i in range(len(importances))],
...
def run_train(args: argparse.Namespace) -> None:
    ...
    target_cfg = TargetConfig(
        args.label_source,
        args.min_confidence_glads2,
        args.min_confidence_gladl,
        args.min_confidence_radd,
        args.pre2020_min_confidence_glads2,
        args.pre2020_min_confidence_radd,
    )

    x, y, groups, summary = build_training_table(
        train_tile_ids,
        aef_dir,
        labels_dir,
        year_map,
        args.baseline_year,
        target_cfg,
        args.max_pos_per_pair,
        args.max_neg_per_pair,
        args.seed,
    )
    ...
    log(
        ...
        f"pre2020_excl={summary['pre2020_excluded_pixels_total'].sum():,} "
        f"disagreement_ignored={summary['disagreement_ignored_pixels_total'].sum():,} "
        f"uncertainty_ignored={summary['uncertainty_ignored_pixels_total'].sum():,} "
        f"sampled={summary['sampled_pixels'].sum():,}"
    )

    fold_df, final_n_estimators = cross_validate_classifier(
        x, y, groups, args.seed, args.cv_folds, args.eval_threshold, args.classifier
    )
    cv_summary_df = summarize_cv_metrics(fold_df)
    final_model = train_final_model(x, y, args.seed, args.classifier, final_n_estimators)

    model_path = output_dir / ("xgboost_model.joblib" if args.classifier == "xgboost" else "linear_model.joblib")
    ...
    if args.save_train_preview:
        log("save_train_preview is not implemented in this XGBoost script; skipping.")

    row = cv_summary_df.iloc[0]
    log(
        f"CV summary: precision={row['precision_mean']:.4f}±{row['precision_std']:.4f} "
...
def predict_tile(
    ...
    for geom in gdf.geometry:
        ...
        if geom.geom_type not in {"Polygon", "MultiPolygon"}:
            continue
        features.append({"type": "Feature", "properties": {}, "geometry": geom.__geo_interface__})
...
def run_predict(args: argparse.Namespace) -> None:
    ...
    if xgb_path.exists():
        ...
        raise FileNotFoundError(f"No saved model found in {output_dir}; expected {xgb_path.name} or {linear_path.name}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="AEF temporal-pairs pipeline with XGBoost or linear classifier")
    ...
    def add_trainish(p: argparse.ArgumentParser):
        p.add_argument("--classifier", default="xgboost", choices=["xgboost", "linear"])
